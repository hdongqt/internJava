...
const bookAPI = {
    ...
    deleteBook(listid){
        return axios.delete(BOOK_BASE_REST_API_URL,{
            headers:authHeader(),
            data: listid
        });
    },
    searchBook(name){
        ...
        return axios.get(BOOK_BASE_REST_API_URL+`/search?${paramString}`, {headers:authHeader()})
    },
    addBookExits(book){
        return axios.put(BOOK_BASE_REST_API_URL,book,{headers:authHeader()})
    }
}
