...
function HistoryModal({historyDetail,onCloseModal,onDeleteHistory}) {
  ...
  return (
        ...
        <button className='btn btn-dark' onClick={()=>{onCloseModal()}}>X</button>
         ...
         <div className="modal-body">
            <div className="row">
              ...
              <div className="col-8">
              <div className="card history-card square scrollbar-cyan bordered-cyan">
                   {
                     historyDetail && historyDetail.listbooks.map(item=>
                      <p key={item.id}>{`- ${item.bookname} - ${item.author}`}</p>
                      )
                   }
               </div>
              </div>
      ...
      <button className='btn btn-danger me-3' onClick={()=>{onDeleteHistory(historyDetail.id)}}>Xóa</button>
