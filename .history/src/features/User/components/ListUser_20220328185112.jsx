 ...
 const s =  users.map(item=>{
  ...
  const deleteUser = () => {
    ...
    if(window.confirm("Xác nhận xóa ?")) {
        UserAPI.deleteUser(listid).then((response) =>{
  ...
  return (
    <div className = "container">
      <h2 className = "text-center"> Danh sách người dùng </h2>
       ...
       onChange={(e)=>{setFilters({...filters,role: e.value,page: 1})}}
       />
     </div>
       <div className="input-group">
        ...
        >
            ...
            placeholder='Nhập tên để tìm kiếm...'
             onChange={(e)=>{setFilters({...filters,fullname: e.target.value,page:1})}}
                       ...
                       users && users.map(
                            ...
                            <tr key = {user.id}> 
                               <td>  
                                ...
                                <td>{user.address}</td>
                                <td>{user.phone}</td>
                                <td className='text-danger'>{user.isDelete ? 'Đã xóa' : ''}</td>
                                <td>
                                    <Link className="btn btn-primary"
                                    style={{width: "max-content"}}
                                    to={`/user/update/${user.id}`} >Chi tiết</Link>
                                </td>
                            </tr>
                        )
                    }
