...
const EditUser = (props) => {
  ...
  useEffect(() => {
      UserAPI.getOneUser(id).then((response) => {
       if(response.data){
        //  const { id, fullname ,username, email, phone, cmnd, sex, birth , address, roleCode, isDelete } = response.data;
           setInfo({...response.data,isDelete: response.data.delete})
          }
        }).catch(error =>{
          const resMessage =
          ...
          <div className="form-group w-25 me-5">
              ...
              <div className="form-group w-25">
                ...
                <Select options={optionsIsDelete}
                  isSearchable={false}
                  value={options.find(item=>item.value==info.isDelete)}
                // onChange={(e)=>{setFilters({...filters,role: e.value})}}
                />
              </div>
          </div>
